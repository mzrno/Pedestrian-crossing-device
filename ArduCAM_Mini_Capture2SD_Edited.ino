...
void loop() {
  State = digitalRead(35); //Read the state on pin 35
  if (State == HIGH) //If the state is HIGH do the following task
  {
    Serial.println("Signal received!");
    myCAMSaveToSDFile(); //Caputure to SD card
    delay(5000);
  }
  else
  {
    Serial.println("There is no signal!");
  }
}
